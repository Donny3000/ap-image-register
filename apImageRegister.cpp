...
bool ImageRegister::setFixedImage(const Mat& fixed_img)
{
  fixed_img.copyTo( fixed );
  return initialize();
}

bool ImageRegister::setMovingImage(const Mat& moving_img)
{
  moving_img.copyTo( moving );
  return initialize();
}
...
Mat ImageRegister::calJointHistogram(Mat image_1, Mat image_2)
{
  ...
  for (int i = 0; i < image_size.height; i++)
  {
    ...
    {
      jpdf.at<double>(image_1.at<uchar>(i,j), image_2.at<uchar>(i,j)) =
          (double) (jpdf.at<double>(image_1.at<uchar>(i,j), image_2.at<uchar>(i,j)) + 1);
    }    
  }
...
double ImageRegister::calEntropy(Mat image)
{ 
  double entropy;
...
double ImageRegister::calJointEntropy(Mat image_1, Mat image_2)
{
  /* TODO
   * > Verify this calculation
   */
...
double ImageRegister::calMutualInformation(Mat image_1, Mat image_2)
{  
  double mi;

  double entropy_1 = calEntropy(image_1);
  double entropy_2 = calEntropy(image_2);
  double joint_entropy = calJointEntropy(image_1, image_2);
